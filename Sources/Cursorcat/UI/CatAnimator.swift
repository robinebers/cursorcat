...
/// Manually-triggerable cat animations. Each runs for a fixed duration and
/// then the animator returns to the default idle loop.
enum CatAnimation {
    case scratching
    case sleeping
    case alert
...
final class CatAnimator {
    ...
    private func tick() {
        ...
        // Roughly every 20 s of pure idleness, auto-pick a small animation.
        if animation == nil, idleTime > 10, Int.random(in: 0..<200) == 0 {
            animation = Bool.random() ? .sleeping : .scratching
            animationFrame = 0
        }
    ...
    private func advance() {
        switch animation {
        case .sleeping:
            // Short "tired" intro, then alternate the 2 sleep frames very
            // slowly so breathing is barely noticeable. Runs for up to an hour
            // unless interrupted by another interaction or state change.
    ...
    private func emitForRunningAnimation(_ animation: CatAnimation, frame: Int) {
        ...
        case .scratching:  emit(sprite: .scratchSelf, frame: frame)
        case .alert:       emit(sprite: .alert, frame: 0)
        case .runAround:
            let frames = CatRenderer.Cell.runAround
